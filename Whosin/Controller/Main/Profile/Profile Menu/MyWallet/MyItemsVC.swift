    ...
    private func _loadData(isLoading: Bool = false) {
        ...
        if isLoading {
            ...
            _vouchersList.forEach { vouchersList in
                if vouchersList.type == "activity", vouchersList.activity?.provider != nil {
                ...
                } else if vouchersList.type == "offer", vouchersList.offer?._venue != nil {
                    if let offerPackagesIds = vouchersList.offer?.packages.map({ $0.id }), (vouchersList.items.reduce(0) { $0 + $1.remainingQty } != 0) {
                ...
                } else if vouchersList.type == "deal", vouchersList.deal?.venueModel != nil {
                    cellData.append([
                        kCellIdentifierKey: kCellIdentifier,
                        kCellTagKey: vouchersList.id,
                        kCellObjectDataKey: vouchersList,
                ...
                } else if vouchersList.type == "event", vouchersList.event?.venueDetail != nil {
...
extension MyItemsVC: CustomTableViewDelegate {
    ...
    func setupCell(_ cell: UITableViewCell, cellDict: [String : Any]?, indexPath: IndexPath) {
        if let cell = cell as? PurchaseVoucherCell {
            ...
            cell.setupData(object, isFrom: "item")
    ...
    func didSelectTableCell(_ cell: UITableViewCell, sectionTitle: String?, cellDict: [String : Any]?, indexPath: IndexPath) {
        if cell is PurchaseVoucherCell {
            guard let object = cellDict?[kCellObjectDataKey] as? VouchersListModel  else { return }
        ...
        } else if cell is MyActivityTableCell {
            guard let object = cellDict?[kCellObjectDataKey] as? VouchersListModel  else { return }
