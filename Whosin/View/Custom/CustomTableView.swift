...
public class CustomTableView: TPKeyboardAvoidingTableView, UITableViewDataSource, UITableViewDelegate {
    ...
    private var _verticleOffSet : CGFloat = 0.0
	...
	public var _cellSectionData: [[String: Any]] = []
    ...
    private var _isAds: Bool = false
    private var _adPossition: Int = 2
    
    private var _adSize: AdSize = .none
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if cellIdentifier == "VideoComponentTableCell" {
            let cachedData = _cachedCell.first{$0.keys.contains{$0 == cacheKey }}
            if let _cachedData = cachedData {
                cell = _cachedData[cacheKey] as! UITableViewCell
            } else {
                cell = Bundle.main.loadNibNamed(String(describing: VideoComponentTableCell.self), owner: nil, options: nil)?[0] as! VideoComponentTableCell
                _cachedCell.append(["\(indexPath.section)-\(indexPath.row)" : cell])
...
//		cell.updateFocusIfNeeded()
    ...
    public func tableView(_: UITableView, viewForHeaderInSection section: Int) -> UIView? {
		if _hasHeaderSection {
            ...
            let sectionBgColor = _cellSectionData[section][kSectionBgColor] as? UIColor ?? .clear
            let sectionFontColor = _cellSectionData[section][kSectionFontColor] as? UIColor ?? .white
            let sectionFontSize = _cellSectionData[section][kSectionFontSize] as? Int ?? 20
    ...
    public func setup(
        ...
        isDummyLoad: Bool = false,
        dummyLoadCount: Int = 0,
		enableRefresh: Bool = false,
        isShowRefreshing: Bool = true,
        emptyDataText: String? = nil,
        emptyDataIconImage: UIImage? = nil,
        emptyDataDescription: String? = nil, delegate: CustomTableViewDelegate, adSize: AdSize = .none, adPossition: Int = 2) {
        ...
        //cellPrototypes?.append(AdsTableCell.cellProtoType)
        ...
        if adSize != .none {
            register(UINib(nibName: AdsTableCell.cellProtoType[kCellNibNameKey] as! String, bundle: nil), forCellReuseIdentifier: AdsTableCell.cellProtoType[kCellIdentifierKey] as! String)
        }
    ...
    public func buildHeaderFooterView(
        ...
        collapsed: Bool = false, barView: Bool = true, rightTitleText: String?, rightTitleColor: UIColor = .white, rightButtonBackgroundColor: UIColor = .white, isNeumorphismView: Bool = false, showRightButton: Bool = false, sectionBgColor: UIColor = .clear, fontSize: CGFloat = 20, textColor: UIColor = .white) -> CollapsibleTableViewHeader {
		let header =  CollapsibleTableViewHeader(
				reuseIdentifier: kHeaderIdentifier,
				collapsible: _isHeaderCollapsible,
				collapsed: collapsed,
                backgroundColor: sectionBgColor, rightButtonColor: rightButtonBackgroundColor, showRightButton: showRightButton)
		...
		header.headerTitleLbl.textColor = _headerFooterTextColor
            header.headerTitleLbl.textColor = textColor
        header.headerTitleLbl.font = FontBrand.SFboldFont(size: fontSize)
		header.section = section
		header.delegate = self
    ...
    public func loadData(_ data: [[String: Any]]?) {
        ...
        if let adList = ADSETTING.adList, !adList.isEmpty {
            if _adSize != .none {
                var index: Int = 0
                _cellSectionData.forEach { section in
                    if var section = section[kSectionDataKey] as? [[String: Any]] {
                        if section.count > _adPossition {
                            section.insert(AdsTableCell.cellData(_adSize), at: _adPossition)
                            _cellSectionData[index][kSectionDataKey] = section
                        }
                    }
                    index += 1
                }
    ...
    public func fetchAndInsertBanner() {
        ADSETTING.requestPromotionBanner { [weak self] bannerCells in
            guard let self = self else { return }

            guard !bannerCells.isEmpty else { return }

            var cellSectionData = self._cellSectionData
            if cellSectionData.isEmpty {
                cellSectionData = [[kSectionTitleKey: kEmptyString, kSectionDataKey: []]]
                return
            }
            guard var cellData = cellSectionData[0][kSectionDataKey] as? [[String: Any]],
                  !cellData.allSatisfy({
                      let identifier = $0[kCellIdentifierKey] as? String
                      return identifier == "EmptyDataCell" || identifier == "LoadingCell"
                  }) else {
                return
            }

            if var cellData = cellSectionData[0][kSectionDataKey] as? [[String: Any]] {
                cellData.removeAll { $0[kCellTagKey] as? String == "promotionBanner" }

                if cellData.count < 5 {
                    if let firstBanner = bannerCells.first {
                        cellData.append(firstBanner)
                    }
                } else {
                    var insertOffset = 0
                    for (index, banner) in bannerCells.enumerated() {
                        let insertIndex = min((index + 1) * 5 + insertOffset, cellData.count)
                        cellData.insert(banner, at: insertIndex)
                        insertOffset += 1
                        if insertIndex >= cellData.count { break } 
                    }
                }

                cellSectionData[0][kSectionDataKey] = cellData
                DISPATCH_ASYNC_MAIN {
                    self.loadData(cellSectionData)
                }
            }
        }
    }
    ...
    public func updateEmptyDataText (title: String?, description: String? , image:UIImage? = nil) {
        _emptyDataText = title
        _emptyDataDescription = description
        _emptyDataIconImage = image
    }
...
extension CustomTableView: DZNEmptyDataSetSource, DZNEmptyDataSetDelegate {
    ...
    public func title(forEmptyDataSet _: UIScrollView!) -> NSAttributedString! {
		...
		return NSAttributedString(string: _emptyDataText ?? kEmptyString, attributes: [
            NSAttributedString.Key.font: FontBrand.labelFont,
            NSAttributedString.Key.foregroundColor: ColorBrand.white.withAlphaComponent(0.7)
        ])
	}
    ...
    public func description(forEmptyDataSet _: UIScrollView!) -> NSAttributedString! {
		...
		return NSAttributedString(string: _emptyDataDescription ?? kEmptyString, attributes: [
			NSAttributedString.Key.font: FontBrand.labelFont,
            NSAttributedString.Key.foregroundColor: ColorBrand.white
		])
	}
    ...
    var verticleOffset: CGFloat {
        get {
            return _verticleOffSet
        }
        set(newWeight) {
            _verticleOffSet = newWeight
        }
    }
