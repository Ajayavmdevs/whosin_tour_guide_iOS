...
//        _usersCollectionView.showsHorizontalScrollIndicator = false
    ...
    private func configureImageViews(imageViews: [UIImageView], userImages: [UserDetailModel]) {
        ...
        let maxVisibleImages = min(totalImageViews, userImages.count)
        ...
        let remainingUserNames = userImages.map { $0.firstName }
    }
...
//    }
    ...
    private func _openOwnerActionSheet() {
        ...
        alert.addAction(UIAlertAction(title: "delete".localized(), style: .destructive, handler: {action in
            DISPATCH_ASYNC_MAIN {
                self.parentBaseController?.showCustomAlert(title: kAppName, message: "delete_bucket_alert".localized(), yesButtonTitle: "yes".localized(), noButtonTitle: "cancel".localized(), okHandler: { UIAlertAction in
    ...
    private func _openShareUserActionSheet() {
        ...
        alert.addAction(UIAlertAction(title: "exit_from_bucket".localized(), style: .destructive, handler: {action in
            DISPATCH_ASYNC_MAIN {
                self.parentBaseController?.showCustomAlert(title: kAppName, message: "exit_from_bucket_alert".localized(), yesButtonTitle: "yes".localized(), noButtonTitle: "cancel".localized(), okHandler: { UIAlertAction in
    ...
    public func setupData(_ model: BucketDetailModel, userModel: [UserDetailModel] = [], isFromSheet: Bool = false) {
        ...
        _bucketCreatedDate.text = "\(model.items.count) offers"
        bucketId = model.id
    }
...
extension BucketListTableCell: UIViewControllerTransitioningDelegate {
...
extension BucketListTableCell: CustomCollectionViewDelegate {
