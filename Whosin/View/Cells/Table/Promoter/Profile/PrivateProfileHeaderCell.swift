...
class PrivateProfileHeaderCell: UITableViewCell {
...
//        }
        ...
        _setupButtonOption(isSubAdmin)
        _editProfileBtn.isHidden = Preferences.isSubAdmin
    }
        ...
        if isSubAdmin {
            ...
            if !self._isComplemenatary  {
                ...
                if _isPublic {
                    ...
                    updateButtonForJoinRing(promoterModel?.profile?.myRingStatus ?? "none", isCM: self._isComplemenatary)
    ...
    private func _openChat() {
        ...
        let chatIds = [user.userId, userDetail.id].sorted()
...
extension PrivateProfileHeaderCell: ReloadProfileDelegate {
