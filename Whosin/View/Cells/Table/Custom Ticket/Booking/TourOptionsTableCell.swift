...
class TourOptionsTableCell: UITableViewCell {
    ...
    private func _setup() {
        DISPATCH_ASYNC_MAIN_AFTER(0.01) {
            self._customBadgeView.layer.maskedCorners = [.layerMinXMaxYCorner]
            self._customBadgeView.layer.cornerRadius = 10
            self._discountView.layer.maskedCorners = [.layerMinXMaxYCorner, .layerMaxXMaxYCorner]
            self._discountView.layer.cornerRadius = 10
        }
        _customAddOnView.reloadCallback = { [weak self] in
...
//        var parts: [String] = []
//        if min > 0 {
//            parts.append(LANGMANAGER.localizedString(forKey: "note__min_text", arguments: ["value": "\(min)"]))
//        }
//        if max < 1000 && max != 0 {
//            parts.append(LANGMANAGER.localizedString(forKey: "note__max_text", arguments: ["value": "\(max)"]))
//        }
//
//        if parts.isEmpty {
//            _noteText.isHidden = true
//        } else {
//            _noteText.isHidden = false
//            _noteText.text = LANGMANAGER.localizedString(forKey: "note_final", arguments: ["value": parts.joined(separator: " and ")])
//        }
    }
...
//        _tourDesc.text = options.descriptions
        ...
        if let selected = BOOKINGMANAGER.bookingModel.tourDetails.first(where: { $0.optionId == options._id }) {
            _customAddOnView.setupData(model: addOnOptions, selected)
        } else {
            _customAddOnView.setupData(model: addOnOptions, nil)
        }
        _customAddOnView.isHidden = addOnOptions.count == 0
...
//        _tourDesc.text = Utils.convertHTMLToPlainText(from: options.descriptionText)
        ...
        DISPATCH_ASYNC_MAIN_AFTER(0.01) {
            self._customBadgeView.setupWithoutDecimalData(originalPrice: originalPrice, discountedPrice: discountPrice, isNoDiscount: options.pricingPeriods.first?.hasDiscount == true)
        }
    ...
    @IBAction private func _handleDateSelectEvent(_ sender: UIButton) {
        ...
        controller.didUpdateCallback = { [weak self] date, time in
            ...
            if let selectedSlot = time as? TourTimeSlotModel {
                ...
                if let option = _selectedTourOptionModel {
                    let oldTourDate = BOOKINGMANAGER.bookingModel.tourDetails.first(where: { $0.optionId == option._id })?.tourDate
                    let isclearAddon = Utils.dateToString(Utils.stringToDate(selected, format: kFormatDateLocal), format: kFormatDate) != oldTourDate
                    BOOKINGMANAGER.addOption(option, adult: adult, child: child, infant: infant, date: Utils.dateToString(Utils.stringToDate(selected, format: kFormatDateLocal), format: kFormatDate), timeSlot: selectedSlot, pickup: _selectedPickup, clearAddons: isclearAddon)
                    callback?()
                }
                ...
                if let option = _selectedBigBusOptionModel {
                    ...
                    BOOKINGMANAGER.addOption(option, adult: adult, child: child, infant: infant, date: Utils.dateToString(Utils.stringToDate(selected, format: kFormatDateLocal), format: kFormatDate), timeSlot: model, optionDetail: BOOKINGMANAGER.bookingModel.tourDetails.first(where: { $0.optionId == "\(option.id)" }), pickup: _selectedPickup)
                    callback?()
                }
            }
        }
...
extension TourOptionsTableCell: ExpandableLabelDelegate {
